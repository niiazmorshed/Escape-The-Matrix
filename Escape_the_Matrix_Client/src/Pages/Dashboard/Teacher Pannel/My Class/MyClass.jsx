...
import { FaBookOpen, FaChalkboardTeacher, FaPlus, FaUsers } from "react-icons/fa";
...
const MyClass = () => {
  ...
  const filtered = useMemo(() => myClass.filter((c) => getStatus(c) === tab), [myClass, tab]);
    ...
    <div className="min-h-screen">
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-gray-600 dark:text-gray-300 text-lg">Manage your teaching courses</p>
            </div>
          </div>
          <button onClick={() => window.location.assign('/dashboard/addclass')} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-xl font-semibold flex items-center space-x-2 hover:shadow-lg transition-all duration-300 transform hover:scale-105">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg border border-gray-100 dark:border-gray-700 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-blue-600 dark:text-blue-400">{myClass.length}</p>
              </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg border border-gray-100 dark:border-gray-700 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-600 dark:text-gray-300 text-sm font-medium">Total Students</p>
      ...
      <div className="flex items-center gap-2 mb-4">
        {['approved','rejected','pending'].map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`px-3 py-2 rounded-lg text-sm font-semibold border transition-colors ${tab===t ? 'bg-blue-600 text-white border-blue-600' : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 border-gray-200 dark:border-gray-700 hover:bg-blue-50 dark:hover:bg-gray-700'}`}
          >
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-16">
          ...
          <p className="text-gray-500 mb-6">Try a different filter or create a new class</p>
